...
Publisher.prototype.on = function() {
	...
	client.on('connect', function(re, err){
		...
		counter.run(function(data) {
			try{
				charts.standardCycleTime = data['Statistics'][0][0].SCT;
				triangle = data['Statistics'][1];
				charts.recent20AverageTime = data['Statistics'][2];
				charts.indexFor3MonthTop = data['Statistics'][3];
				charts.dailyAverageTop3 = data['Statistics'][4];
